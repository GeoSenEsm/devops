...
SQL Server runs on a separate host. MongoDB (response documents) runs in
Docker on the app server next to the API and admin panel.
...
- `JWT_EXPIRATION`: JWT lifetime in days
...
MongoDB URI and database name are set in Compose (`SPRING_DATA_MONGODB_URI` /
`SPRING_DATA_MONGODB_DATABASE`) and do not need to be in `.env` unless you
change them.
